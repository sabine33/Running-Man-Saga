...
---Loads Music
local music = audio.loadStream("music/music.mp3")
audio.setVolume(0.5, {channel = 1}) -- set the volume on channel 1
local backgroundMusicChannel = audio.play(music, {
    channel = 1,
    loops = -1,
    fadein = 3000
})
local bombSound = audio.loadSound("music/bomb.wav")
audio.setVolume(0.2, {channel = 2}) -- set the volume on channel 1
local coinSound = audio.loadSound("music/coin.mp3")
audio.setVolume(1, {channel = 3, loops = 0, fadein = 1000})
local manGrunt = audio.loadSound("music/manGrunt.mp3")
audio.setVolume(1, {channel = 4, loops = 0, fadein = 1000})
...
function chooseBg()
    ...
    for i = 1, 3 do
        ...
        bgs[i].height = 320
        bgs[1].x = 0
        if (i > 1) then bgs[i].x = bgs[i - 1].x + bgs[i - 1].width end
        bgs[i].y = 0
    end
end

-- Load Score in Screen
function loadScore()
    scoreImage = display.newImageRect("img/coin.png", 25, 25)
    scoreImage.x = 0
    scoreImage.y = 30
    scoreText = display.newText(" X " .. score .. "/" .. highScore, 15, 30,
                                system.nativeFontBold, 20)
    scoreText:setFillColor(1, 0, 0)
    lifeScore = display.newImage("img/life.png", width - 80, 30)
    lifeScore:scale(0.3, 0.3)
    lifeText = display.newText(life, width - 65, 35, system.nativeFontBold, 20)
    levelText = display.newText("LEVEL : " .. level, width / 2 + 100, 30,
                                system.nativeFontBold, 20)
...
function loadItems()
    ...
    bomb = {}
    coins = {}
    local coin = graphics.newImageSheet("img/coins.png", {
        width = 40,
        height = 178 / 4,
        numFrames = 4
    })
    for i = 1, 3 do
        math.randomseed(os.time())
        coins[i] = display.newSprite(coin, {
            name = "coin",
            start = 1,
            count = 4,
            time = 300
        })
        bar[i] = display.newImage("img/bar.png", 300, 245)
        bar[i]:scale(0.3, 0.2)
        bomb[i] = display.newImage("img/bomb.png", 300, 245)
        bomb[i]:scale(0.3, 0.3)
        bar[i].x = math.random(1, 4) * i * math.random(400, 600) + i *
                       math.random(width, width + 300)
        coins[i].x = math.random(2, 3) * i * math.random(100, 400) + i *
                         math.random(width + 30, width + 300)
        coins[i].y = math.random(200, 230)
        coins[i]:scale(0.8, 0.8)
    end
...
function actorLoad()
    ...
    sheet2 = graphics.newImageSheet("img/anim.png", {
        width = 512 / 6,
        height = 512 / 5,
        numFrames = 30
    })
    actor = display.newSprite(sheet2, sequenceData)
    actor.x = 100
    actor.y = 110
    actor:scale(0.6, 0.6)
    actorStop = display.newSprite(sheet2, {start = 5, count = 1})
    actorStop:scale(0.6, 0.6)
    actorStop.y = actor.y
    actorStop.x = actor.x
    actorStop.alpha = 0
    actor.y = height
...
skyBg = display.newImageRect("img/sky.jpg", display.contentWidth,
...
local function move(event)
    ...
    if (grass.x + grass.contentWidth) < 0 then grass:translate(480 * 2, 0) end
    if (grass2.x + grass2.contentWidth) < 0 then grass2:translate(480 * 2, 0) end
    for i = 1, 3 do
        bgs[i]:translate(-1, 0)
        ...
        if (bgs[i].x < -bgs[i].width) then bgs[i].x = width + 10 end

        if ((actor.x + actor.contentWidth) >= bar[i].x and
            (actor.x + actor.contentWidth) <= (bar[i].x + bar[i].contentWidth) and
            (actor.y + actor.contentHeight) >= bar[i].y and actor.y <=
            (bar[i].y + bar[i].contentHeight)) then
            audio.play(manGrunt)
        ...
        if ((actor.x + actor.contentWidth) >= love.x and
            (actor.x + actor.contentWidth) <= (love.x + love.contentWidth)) then
            life = life + 1
            love.x = 6000
        end
        if ((actor.x + actor.contentWidth) >= coins[i].x and
            (actor.x + actor.contentWidth) <=
            (coins[i].x + coins[i].contentWidth) and actor.y <=
            (coins[i].y + coins[i].contentHeight)) then
   ...
   display.newText( "Jump Dude", 200,200,system.nativeFontBold,30)
    ...
    if (actor.y <= 110) then actor.y = 110 end
    if ((actor.y + actor.contentHeight) <= (220)) then jump = false end
...
function showTitleScreen(type)
    if (type == "main") then
        ...
        titleView =
            display.newGroup(title, playBtn, scoreBtn, helpBtn, aboutBtn)
    elseif type == "score" then
        scaletoZero()
        scoreView = display.newText("HighScore : " .. highScore, 100, 100,
                                    system.nativeFontBold, 30)
...
function helpMenu() showTitleScreen("help") end
function scoreMenu() showTitleScreen("score") end

function aboutMenu(event) showTitleScreen("about") end
function play(event)
    score = 0
    life = 3
...
-- CALLING FUNCTIONS
function Main() showTitleScreen("main") end
